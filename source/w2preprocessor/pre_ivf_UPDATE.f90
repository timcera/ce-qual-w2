  ...
  DO JW=1,NWB
   ...
   !   READ (NPT,'(//)')
      ...
      DO WHILE (.TRUE.)
        ...
        END IF
        IF (TAIR == 0.0 .AND. phi == 0.0 .AND. ABS(TAIR-TAIRO) > 5.0) THEN
          CALL WARNINGS
...
! Vector output
  ...
  DO JW=1,NWB
    IF ((VPLC(JW) /= '      ON') .AND. (VPLC(JW) /= '     OFF')) THEN
      CALL ERRORS
      WRITE (ERR,FMTI) 'Vector output: Vector plot control [VPLC='//VPLC(JW)(6:8),'] must be either " ON" or "OFF" for waterbody ',JW
    END IF
    IF (VPLC(JW) == '      ON') THEN
      IF (NVPL(JW) <= 0) THEN
        CALL WARNINGS
        WRITE (WRN,FMTI) 'Vector output: Number of vector plot dates [NVPL=',NVPL(JW),'] <= 0 for waterbody ',JW
      END IF
      DO J=1,NVPL(JW)-1
        IF (VPLD(J+1,JW) <= VPLD(J,JW)) THEN
          CALL ERRORS
          WRITE (ERR,FMT2FI) 'Vector output: Vector plot date [VPLD=',VPLD(J,JW),'] <= previous date [VPLD=',VPLD(J-1,JW),'] in waterbody ',JW
        END IF
      END DO
      DO J=1,NVPL(JW)
...
! Initial input and conditions
  ...
  WRITE(INI,'(//"Meterological Data Input Summary",/,"    Parameter   Waterbody   Average Value    Maximum     Minimum",/)')
  DO JW=1,NWB
      ...
      write(INI,'(4x,"CLOUD(1-10)",4X,I3,7X,F10.3,3X,F10.3,2X,F10.3)')JW,cloudav(jw),cloudmax(jw),cloudmin(jw)
