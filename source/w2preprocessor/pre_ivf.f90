...
MODULE CONTROL
  ...
  LOGICAL                                          :: DELETE_ERR, DELETE_WRN,DOWNSTREAMDAM  ! POLYGONS
END MODULE CONTROL
MODULE GRIDC
...
PROGRAM W2_PRE
  ...
  USE GRID
  USE EPIPHYTON; USE ENVIRPMOD
! Variable declarations

  TYPE (WINDOWCONFIG) WC
  TYPE (QWINFO)       WINFO
  CHARACTER(260)::  MODDIR, DIRC   ! SW 11/9/2017 If you are likely to encounter a long path, allocate a buffer of size $MAXPATH ($MAXPATH = 260). USE IFPORT CHARACTER($MAXPATH) dir dir = FILE$CURDRIVE length = GETDRIVEDIRQQ(dir)
  REAL          :: JDAY,   JDAYO
  ...
  CHARACTER(289) :: WINTITLE                                                                                            !SW 10/15/00
  ...
  CHARACTER(3) :: PARTON,ALINE,DXTHEORYC
  REAL :: OUTFREQP
...
! Allocation declarations
  ...
  LOGICAL                                      :: CONSTITUENTS,     BRANCH_FOUND, FISH_PARTICLE_EXIST                                       !SW 10/17/02
...
length = GETDRIVEDIRQQ(moddir)
  ...
  STAT = ABOUTBOXQQ ('CE-QUAL-W2 Preprocessor V4.1 - PSU Dept. Civil & Environmental Engineering S. Wells'C)   
  WRITE (WINTITLE,'(A29,A260)') 'CE-QUAL-W2 4.1 preprocessor. ',moddir                  !A260)') 'CE-QUAL-W2 4.1 preprocessor. ',moddir
...
unit=5000
  ...
  ELSE
    ! check file
      ...
      READ (unit,1200,ERR=400,END=5001) numclass,selectivec,sjday1,sjday2,istart,iend
      N=1
      IF(NUMCLASS>50)THEN
        CALL WARNINGS
      ...
      ENDIF
       ...
       IF(istart>imx)THEN
        CALL ERRORS
        WRITE(ERR,'(a45,i4,a6,i4)')'ENVIRPC: ISTR MUST BE LESS THAN IMX. ISTART=',ISTART,'>IMX=',IMX
       ENDIF
       IF(istart>iend)THEN
        CALL ERRORS
        WRITE(ERR,'(a45,i4,a6,i4)')'ENVIRPC: ISTR MUST BE LESS THAN IEND. ISTART=',ISTART,'>IEND=',IEND
       ENDIF
...
! Sediment Diagenesis Check
 ...
 If(file_exists) Then	
	 WRITE (WIN,*) 'Reading W2 Sediment Diagenesis File: w2_CEMA.npt'  
    ...
    if(nh2s < NGCS  .or. nh2s > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for hydrogen sulfide in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nch4
    if(nch4 < NGCS  .or. nch4 > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for methane in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nso4    
    if(nso4 < NGCS  .or. nso4 > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for sulfate in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nturb
    if(nturb < NGCS  .or. nturb > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for turbidity in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nFe2
    if(nFe2 < NGCS  .or. nFe2 > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for Fe(II) in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nFeOOH
    if(nFeOOH < NGCS  .or. nFeOOH > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for FeOOH in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nMn2
    if(nMn2 < NGCS  .or. nMn2 > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for Mn(II) in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nMnO2
    if(nMnO2 < NGCS  .or. nMnO2 > NGCE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for MnO2 in w2_CEMA.npt file should be >=',NGCS,' and <=',NGCE
    endif
    Read(5000,*)MessageTemp, nmft
    if(nmft < NSSS  .or. nmft > NSSE)then
        CALL ERRORS
        WRITE(ERR,FMTI)'Sediment Diagenesis:Constituent Number specified for mature fine tailings in w2_CEMA.npt file should be >=',NSSS,' and <=',NSSE
    endif
    ngh2s=nh2s-NGCS+1
    ngch4=nch4-NGCS+1
    ngso4=nso4-NGCS+1
    ...
    IF (CEMAPWpH <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTFI) 'Sediment Diagenesis:Porewater pH ',        CEMAPWpH,' <= 0.0 '
    END IF
    ...
    IF (IncludeDynamicpH .and. .not. IncludeAlkalinity) THEN
      CALL ERRORS
      WRITE (ERR,*) 'Sediment Diagenesis:Modeling of alkalinity in sediments required if dynamic modeling of pH in sediments is on'
    END IF
    ...
    !If(IncludeCEMAGenBODConstituents)Then
        ...
        IF (IncludeCEMAGenBODConstituents .and. NumGenBODConstituents <= 0) THEN
         CALL ERRORS
         WRITE (ERR,FMTI) 'Sediment Diagenesis:Generic sediment BOD constituents are turned on but the number of generic sediment BOD constituents ',        NumGenBODConstituents,' <= 0 '
        END IF
        IF (IncludeCEMAGenBODConstituents .and.  NumGenBODConstituents > NBOD) THEN
         CALL ERRORS
         WRITE (ERR,'(A,I4,A,I4)') 'Sediment Diagenesis:Generic sediment BOD constituents are turned on but the number of generic sediment BOD constituents ',        NumGenBODConstituents,' > NBOD (the number of BOD compartments):',nbod
        END IF
        ...
        IF (IncludeCEMAGenBODConstituents .and. NumGenBODInitRegns <= 0) THEN
         CALL ERRORS
         WRITE (ERR,FMTI) 'Sediment Diagenesis:Generic sediment BOD constituents are turned on but the number of regions for initializing generic sediment BOD constituents ',        NumGenBODConstituents,' <= 0 '
        END IF
        ...
        do i=1,NumGenBODInitRegns
          if(SedGenBODRegSegSt(i) < 1 .or. SedGenBODRegSegSt(i) > IMX)then
            CALL ERRORS
            WRITE(ERR,FMTI)'Sediment Diagenesis:Starting segment in generic sediment BOD region outside model grid: I=',SedGenBODRegSegSt(i),' is less than 1 or greater than IMX=',imx
          endif
        end do
    	...
    	do i=1,NumGenBODInitRegns
          if(SedGenBODRegSegEn(i) < 1 .or. SedGenBODRegSegEn(i) > IMX)then
            CALL ERRORS
            WRITE(ERR,FMTI)'Sediment Diagenesis:Ending segment in generic sediment BOD region outside model grid: I=',SedGenBODRegSegEn(i),' is less than 1 or greater than IMX=',imx
          endif
          if(SedGenBODRegSegSt(i) > SedGenBODRegSegEn(i))then
            CALL ERRORS
            WRITE(ERR,FMTI)'Sediment Diagenesis:Starting segment in generic sediment BOD region  I=',SedGenBODRegSegSt(i),' is greater than ending segment I=',SedGenBODRegSegEn(i)
          endif
       end do
        ...
        do i=1,NumGenBODConstituents
          do j=1,NumGenBODInitRegns
            IF (SedGenBODInit(i,j) < 0.0) THEN
              CALL ERRORS
              WRITE (ERR,'(2(A,I0),A,F0.3)') 'Sediment Diagenesis:Initial concentration for generic sediment BOD ',i,' and region ',j, ' is ',SedGenBODInit(i,j),' and < 0.0 '
            ELSE IF(SedGenBODInit(i,j)== 0.0)then
              CALL warnings
              WRITE (wrn,FMTI) 'Sediment Diagenesis:Initial concentration for generic sediment BOD ',i,' and region ',j, ' is 0'
            end if
          end do
        end do
        ...
        IF (IncludeCEMAGenBODConstituents .and. NumGenBODConsumptionRegions <= 0) THEN
         CALL ERRORS
         WRITE (ERR,FMTI) 'Sediment Diagenesis:Generic sediment BOD constituents are turned on but number of regions for different generic sediment BOD constituent consumption rate ',        NumGenBODConstituents,' <= 0 '
        END IF
        ...
        do i=1,NumGenBODConsumptionRegions
          if(SedGenBODConsRegSegSt(i) < 1 .or. SedGenBODConsRegSegSt(i) > IMX)then
            CALL ERRORS
            WRITE(ERR,FMTI)'Sediment Diagenesis:Starting segment for generic sediment BOD consumption region outside model grid: I=',SedGenBODConsRegSegSt(i),' is less than 1 or greater than IMX=',imx
          endif
        end do
        ...
        do i=1,NumGenBODConsumptionRegions
          if(SedGenBODConsRegSegEn(i) < 1 .or. SedGenBODConsRegSegEn(i) > IMX)then
            CALL ERRORS
            WRITE(ERR,FMTI)'Sediment Diagenesis:Ending segment in generic sediment BOD consumption region outside model grid: I=',SedGenBODConsRegSegEn(i),' is less than 1 or greater than IMX=',imx
          endif
          if( SedGenBODConsRegSegSt(i)  > SedGenBODConsRegSegEn(i))then
            CALL ERRORS
            WRITE(ERR,FMTI)'Sediment Diagenesis:Starting segment in generic sediment BOD consumption region outside model grid: I=', SedGenBODConsRegSegSt(i) ,' is greater than ending segment I=',SedGenBODConsRegSegEn(i)
          endif
       end do        
        Do i = 1, NumGenBODConstituents
        ...
        do i=1,NumGenBODConstituents
          do j=1,NumGenBODInitRegns
            IF (SedGenBODRegnRate(i,j) < 0.0) THEN
              CALL ERRORS
              WRITE (ERR,'(2(A,I0),A,F0.3)') 'Sediment Diagenesis:Consumption rate for generic BOD ',i,' and region ',j, ' is ',SedGenBODRegnRate(i,j),' and < 0.0 '
            ELSE IF(SedGenBODRegnRate(i,j)== 0.0)then
              CALL warnings
              WRITE (wrn,FMTI) 'Sediment Diagenesis:Consumption rate for generic BOD ',i,' and region',j, ' is 0'
            end if
          end do
        end do                
	    Do i = 1, NumGenBODConstituents
        ...
        do i=1,NumGenBODConstituents
          do j=1,NumGenBODInitRegns
            IF (SedGenBODRegnTCoeff(i,j) < 0.0) THEN
              CALL ERRORS
              WRITE (ERR,'(2(A,I0),A,F0.3)') 'Sediment Diagenesis:Temperature coefficient for generic BOD decay for BOD# ',i,' and region ',j, ' is ',SedGenBODRegnTCoeff(i,j),' and < 0.0 '
            ELSE IF(SedGenBODRegnTCoeff(i,j)== 0.0)then
              CALL warnings
              WRITE (wrn,FMTI) 'Sediment Diagenesis:Temperature coefficient for generic BOD decay for BOD# ',i,' and region',j, ' is 0'
            end if
          end do
        end do                
    !End If
    ...
    do i=1,NumRegnsSedimentDiagenesis
      IF (SDRegnPOC_L_Fr(i) < 0.0 .or. SDRegnPOC_L_Fr(i) >1.0) THEN
        CALL ERRORS
        WRITE (ERR,FMTIF) 'Sediment Diagenesis:Fraction of labile poc for diagenesis region ',i, ' is ',SDRegnPOC_L_Fr(i),' ,which is < 0.0 or > 1.0'
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegnAe_CH4_CO2(i) > 3.0)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Methane oxidation rate in aerobic layer for region ',i, ' is greater than 3.0'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegnAe_HS_O2_Nit(i) > 3.0)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Nitrification half-saturation constant for O2 in aerobic layer mgO2/l for region ',i, ' is greater than 3.0'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegnAe_H2S_SO4(i) > 3.0)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Sulfide oxidation rate in aerobic layer m/d for region ',i, ' is greater than 3.0'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_PON_Lab(i) > 0.3)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for labile PON 1/d for region ',i, ' is greater than 0.3'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_PON_Ref(i) > 0.01)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for refractory PON 1/d for region ',i, ' is greater than 0.01'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_PON_Ine(i) > 0.001)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for inert/slow refractory PON 1/d for region ',i, ' is greater than 0.001'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_POC_Lab(i) > 0.3)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for labile POC 1/d for region ',i, ' is greater than 0.3'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_POC_Ref(i) > 0.01)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for refractory POC 1/d for region ',i, ' is greater than 0.01'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_POP_Lab(i) > 0.3)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for labile POP 1/d for region ',i, ' is greater than 0.3'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_POP_Ref(i) > 0.01)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for refractory POP 1/d for region ',i, ' is greater than 0.01'
      end if
    end do
    ...
    do i=1,NumRegnsSedimentDiagenesis
      ...
      ELSE IF(SDRegn_MinRate_POP_Ine(i) > 0.001)then
        CALL warnings
        WRITE (wrn,FMTIF) 'Sediment Diagenesis:Mineralization rate for inert/slow refractory POP 1/d for region ',i, ' is greater than 0.001'
      end if
    end do
    ...
    Read(5000,*)MessageTemp, o2h2s    
      IF (o2h2s <= 0.0) THEN
        CALL ERRORS
        WRITE (ERR,FMTF) 'Sediment Diagenesis:Oxygen stoichiometry for H2S decay in water column is ',o2h2s,', which is <= 0.0'
      end if    
    Read(5000,*)MessageTemp, o2ch4    
      IF (o2ch4 <= 0.0) THEN
        CALL ERRORS
        WRITE (ERR,FMTF) 'Sediment Diagenesis:Oxygen stoichiometry for CH4 decay in water column is ',o2ch4,', which is <= 0.0'
      end if    
    Read(5000,*)MessageTemp, Kdp1   
      IF (Kdp1 <= 0.0) THEN
        CALL ERRORS
        WRITE (ERR,FMTF) 'Sediment Diagenesis:Phosphorus Sorption Coefficient in Aerobic Layer m^3/g is ',Kdp1,', which is <= 0.0'
      end if   
    Read(5000,*)MessageTemp, Kdp2
      IF (Kdp2 <= 0.0) THEN
        CALL ERRORS
        WRITE (ERR,FMTF) 'Sediment Diagenesis:Phosphorus Sorption Coefficient in Anaerobic Layer m^3/g is ',Kdp2,', which is <= 0.0'
      end if   
    ...
    IF (NH4_NH3_Eqb_Const <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Equilibrium constant for NH4+ <-> NH3 ionization is ',NH4_NH3_Eqb_Const,', which is <= 0.0'
    end if    
    Read(5000,*)MessageTemp, HS_H2S_Eqb_Const
    IF ( HS_H2S_Eqb_Const <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Equilibrium constant for HS- <-> H2S ionization is ', HS_H2S_Eqb_Const,', which is <= 0.0'
    end if  
    Read(5000,*)MessageTemp, HenryConst_NH3
    IF ( HenryConst_NH3 <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Henrys constant for NH3d <-> NH3g in atm/M is ', HenryConst_NH3,', which is <= 0.0'
    end if  
    Read(5000,*)MessageTemp, HenryConst_CH4
    IF ( HenryConst_CH4 <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Henrys constant for CH4d <-> CH4g in atm/M is ', HenryConst_CH4,', which is <= 0.0'
    end if  
    Read(5000,*)MessageTemp, HenryConst_H2S
    IF ( HenryConst_H2S <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Henrys constant for H2Sd <-> H2Sg in atm/M is ', HenryConst_H2S,', which is <= 0.0'
    end if  
    Read(5000,*)MessageTemp, HenryConst_CO2
    IF ( HenryConst_CO2 <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Henrys constant for CO2d <-> CO2g in atm/M is ', HenryConst_CO2,', which is <= 0.0'
    end if  
    ...
    Read(5000,*)MessageTemp, CoeffB_Turb
     IF (CoeffB_Turb <= 0.0) THEN
      CALL warnings
      WRITE (wrn,FMTF) 'Sediment Diagenesis:Coefficient Bturb in Aturb*ln(TSS) + Bturb = ln(Turbidity) is',CoeffB_Turb,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, WriteCEMAMFTSedFlx    
    Read(5000,*)MessageTemp, PartMixVel
    IF (PartMixVel <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Particle Mixing Velocity between Aerobic and Anaerobic Layers m/d is',PartMixVel,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, BurialVel
     IF (BurialVel <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Sediment Diagenesis burial Velocity m/d is',BurialVel,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, IncludeIron
    Read(5000,*)MessageTemp, IncludeManganese
    ...
    IF (KFeOOH_HalfSat <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Half-saturation constant for O2 for FeOOH reduction to Fe(II) g/m^3 is',KFeOOH_HalfSat,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, kfe_red
    IF (kfe_red <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Reduction rate, FeOOH to Fe(II) d-1 is',kfe_red,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, kfe_oxid
    IF (kfe_oxid <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Oxidation rate, Fe(II) to FeOOH m^3/d-g is',kfe_oxid,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, FeSetVel
    IF (FeSetVel <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:FeOOH Settling Velocity m/d is',FeSetVel,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, KdFe1
    IF (KdFe1 < 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Fe(II) Sorption Coefficient in Aerobic Layer m^3/g is',KdFe1,', which is < 0.0'
    end if
    Read(5000,*)MessageTemp, KdFe2
    IF (KdFe2 < 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Fe(II) Sorption Coefficient in Anaerobic Layer m^3/g is',KdFe2,', which is < 0.0'
    end if
    Read(5000,*)MessageTemp, o2fe2
    IF (o2fe2 <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Oxygen stoichiometry for Fe(II) decay gO/gFe is',o2fe2,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, KMnO2_HalfSat
    IF (KMnO2_HalfSat <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Half-saturation constant for O2 for MnO2 reduction to Mn(II) g/m^3 is',KMnO2_HalfSat,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, kMn_red
    IF (kMn_red <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Reduction rate, MnO2 to Mn(II) d-1 is',kMn_red,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, kMn_oxid
    IF (kMn_oxid <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Oxidation rate, Mn(II) to MnO2 m^3/d-g is',kMn_oxid,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, MnSetVel
    IF (MnSetVel <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:MnO2 Settling Velocity m/d is',MnSetVel,', which is <= 0.0'
    end if
    Read(5000,*)MessageTemp, KdMn1
    IF (KdMn1 < 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Mn(II) Sorption Coefficient in Aerobic Layer m^3/g is',KdMn1,', which is < 0.0'
    end if
    Read(5000,*)MessageTemp, KdMn2
    IF (KdMn2 < 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Mn(II) Sorption Coefficient in Anaerobic Layer m^3/g is',KdMn2,', which is < 0.0'
    end if
    Read(5000,*)MessageTemp, o2Mn2
    IF (o2Mn2 <= 0.0) THEN
      CALL ERRORS
      WRITE (ERR,FMTF) 'Sediment Diagenesis:Oxygen stoichiometry for Mn(II) decay gO/gMn is',o2Mn2,', which is <= 0.0'
    end if
    ...
    Read(5000,*,END=25001)MessageTemp, SEDIAGFREQ 
...
25001 continue
     CALL ERRORS
      WRITE (ERR,'(a105)') 'Sediment Diagenesis:SEDIAGFREQ is not included in last line of CEMAinput.npt [See updated User Manual].'
25002   close(5000)
 End If
...
!Particle tracking Version 4.1 
  ...
  IF(FISH_PARTICLE_EXIST)THEN
   ...
   WRITE (WIN,*) '  Particle Tracking ON: Reading particle.csv'
 
    open(NPT,file='particle.csv',status='old')
    ...
    IF(PARTON /= 'OFF' .AND. PARTON /= 'ON')THEN
    CALL ERRORS
    WRITE (ERR,*) 'Particle.csv file: PARTON must be either OFF or ON. PARTON=',PARTON
    ENDIF
    
    IF(PARTON=='ON')THEN
        IF(NFISHSEG <= 0.0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. NFISHSEG must be > 0. NFISHSEG=',NFISHSEG
        ENDIF
        IF(NFISHPCEL <= 0.0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. NFISHPCEL must be > 0. NFISHPCEL=',NFISHPCEL
        ENDIF
        IF(ALINE /= 'OFF' .AND. ALINE /= 'ON')THEN
        CALL ERRORS
        WRITE (ERR,*) 'Particle.csv file: PARTON is ON. LINE MUST BE EITHER ON OR OFF. LINE=',ALINE
        ENDIF
        IF(DXTHEORYC /= 'OFF' .AND. DXTHEORYC /= 'ON')THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. DXTHEORY MUST BE EITHER ON/OFF. DXTHEORY=',DXTHEORYC
        ENDIF
        IF(OUTFREQP <= 0.0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,F8.3)') 'Particle.csv file: PARTON is ON. OUTFREQP must be > 0. OUTFREQP=',OUTFREQP
        ENDIF
        IF(ILINEAR /= 1 .AND. ILINEAR /= 0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. ILINEAR must be 1 OR 0. ILINEAR=',ILINEAR
        ENDIF
        IF(HTSTBOT /= 1 .AND. HTSTBOT /= 0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. HTSTBOT must be 1 OR 0. HTSTBOT=',HTSTBOT
        ENDIF
        IF(HTSTSIDE /= 1 .AND. HTSTSIDE /= 0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. HTSTSIDE must be 1 OR 0. HTSTSIDE=',HTSTSIDE
        ENDIF
        IF(IDEBUG /= 1 .AND. IDEBUG /= 0)THEN
        CALL ERRORS
        WRITE (ERR,'(A,I4)') 'Particle.csv file: PARTON is ON. IDEBUG must be 1 OR 0. IDEBUG=',IDEBUG
        ENDIF
        ...
        CALL WARNINGS
        WRITE(ERR,'(A,F8.3,A,I4)')'Particle.csv file: PARTON is ON. SEDVELINIT > 25 m/day. SEDVELINIT=',SEDVELINIT,' Particle #:',J
       ENDIF
...
! Spreadsheet output
  ...
  DO JW=1,NWB
    IF ((SPRC(JW) /= '      ON') .AND. (SPRC(JW) /= '     OFF')) THEN
      CALL ERRORS
      WRITE (ERR,FMTI) 'Spreadsheet output: Spreadsheet control [SPRC='//SPRC(JW)(6:8)//'] must be either " ON" or "OFF" for waterbody ',JW
    END IF
    IF (SPRC(JW) == '      ON') THEN
      IF (NSPR(JW) <= 0) THEN
        CALL ERRORS
...
! Initial input and conditions
  ...
  WRITE (INI,'(1X,A/)') 'CE-QUAL-W2 preprocessor - V4.1'
