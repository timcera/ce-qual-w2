...
12      CONTINUE  
        ...
        L      = LEN_TRIM(SEGNUM)  
              OPEN (FLX2(JW), FILE='kflux_wb'//SEGNUM(1:L)//'.opt',POSITION='APPEND')
        JDAY1=0.0  
        REWIND (FLX2(JW))  
        READ   (FLX2(JW),'(/)',END=13)  
        DO WHILE (JDAY1 < JDAY)  
  ...
  ELSE  
    ...
    DO JW=1,NWB  
      ...
      IF (FLUX(JW))THEN  
        ...
        SEGNUM = ADJUSTL(SEGNUM)  
        L      = LEN_TRIM(SEGNUM)  
              OPEN (FLX2(JW), FILE='kflux_wb'//SEGNUM(1:L)//'.opt',STATUS='UNKNOWN')
    ...
    IF (DOWNSTREAM_OUTFLOW) THEN  
      DO JWD=1,NIWDO  
        ...
        IF (CONSTITUENTS) THEN  
          ...
          WRITE (WDO(JWD,3),'(A,I0//(100(A,",")))') '$Concentration file for segment ',IWDO(JWD),'JDAY', (CNAME2(CN(J)),J=1,NAC)                !CNAME2(CN(1:NAC))  
        END IF  
        IF (DERIVED_CALC) THEN  
          OPEN  (WDO(JWD,4),FILE='dwo_'//SEGNUM(1:L)//WDOFN(L1:L1+4),STATUS='UNKNOWN')                 !'.opt',STATUS='UNKNOWN')
          WRITE (WDO(JWD,4),'(A,I0//(100(A,",")))') 'Derived constituent file for segment ',IWDO(JWD),'JDAY', (CDNAME2(CDN(J,JW)),J=1,NACD(JW))                 !CDNAME2(CDN(1:NACD(JW),JW))  
        END IF  
      END DO  
    END IF  
          ...
          IF(IWDO(JWD)==DS(JB) .AND. NSTR(JB) /= 0)THEN  
            ...
            OPEN(WDO2(JFILE,2),FILE='two_str'//SEGNUM(1:L)//'_seg'//SEGNUM2(1:L2)//WDOFN(L1:L1+4),STATUS='UNKNOWN')      !'.opt',STATUS='UNKNOWN')
              ...
              IF (CONSTITUENTS) THEN  
                ...
                WRITE (WDO2(JFILE,3),'(A//(100(A,",")))') TITLEWITH,'JDAY',(CNAME2(CN(J)),J=1,NAC)     !CNAME2(CN(1:NAC))  
              ENDIF  
              IF (DERIVED_CALC) THEN  
                WDO2(JFILE,4) = NUNIT; NUNIT = NUNIT+1  
                ...
                DO JW=1,NWB  
                  IF (IWDO(JWD) >= US(BS(JW)) .AND. IWDO(JWD) <= DS(BE(JW))) EXIT  
                END DO  
                WRITE (WDO2(JFILE,4),'(A//(100(A,",")))') TITLEWITH,'JDAY',(CDNAME2(CDN(J,JW)),J=1,NACD(JW))  !CDNAME2(CDN(1:NACD(JW),JW))  
              ENDIF  
            ENDDO  
          ENDIF  
        ENDDO  
    ...
    ENDDO  
      ...
      IF(MASS_BALANCE(JW).AND.CONTOUR(JW).AND.DERIVED_CALC)THEN  
        ...
        IF(SEDIMENT_DIAGENESIS)THEN
            WRITE(MASSBFN,'(A480)')'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),SED+SOD_PRelease(kg),PFluxtoSediments(kg),SedimentDiagenesisPFlux(kg),TN-Waterbody(kg),TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),SED+SOD_NRelease(kg),NFluxtoSediments(kg),SedimentDiagenesisNH4Flux(kg),SedimentDiagenesisNO3Flux(kg)'   ! 'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),BurialP_SED(kg),SED+SOD_PRelease(kg),TN-Waterbody(kg),TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),BurialN_SED(kg),SED+SOD_NRelease(kg)'
            ELSE
