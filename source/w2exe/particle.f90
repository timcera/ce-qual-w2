...
Module Fishy
      ...
      LOGICAL         particle,line,HITSTICKBOTTOM,HITSTICKSIDE
      character*3     PARTON,collector,partmod,DXTHEORY         ! SW 2/16/01 7/25/2017
      ...
      integer, allocatable, dimension  (:) :: ifish,ifisht,ifishb,GROUP  ! SW 1/16/01
      ...
      INTEGER :: DIAGFN=8001, DATADEBUGFN=8002, BARCHRTXFN=8003,BARCHRTZFN=8004,FINALFN=8005,INITIALFN=8006
...
!      FINDNEWBR,FISHPLOT,INTERCONST,INTERFLOWF
     ...
     USE SURFHE; Use Fishy; Use GDAYC;  Use SCREENC; Use GEOMC; USE GLOBAL;   Use MAIN, only: IWD, FISH_PARTICLE_EXIST 
     IMPLICIT NONE
...
! This first section below checks and corrects fish vertically above or below the bottom
   70   IF ((FKMP.Le.KTI(FIMP)).OR.&   ! SW 2/01/01 Change so that do a check if in "air" or not
          ...
          IF (FKMP.GT.KB(FIMP)) THEN                   !  FKMP is below KB 
            FKMP = FKMP - 1
...
! final fish output
    write(FINALFN,'(a291)')'Part#,Seg#,XLocationwithinSegmentfromUpstreamSide(m),Layer#,VerticalDistfromTop(m),LateralDistfromLeftBank,Branch#,ParticleInModel(=0),JDAYleftsystem,DetentionTime(days),RemovalMechanism,SedVelocity(m/d),DateStart'
    do jf=1,nfish     
    write(FINALFN,'(i7,",",1x,10(f10.3,","),f8.4,",",f12.4)')jf,fishes(jf,1),fishes(jf,2),fishes(jf,3),fishes(jf,4),fishes(jf,5),fishes(jf,6),fishes(jf,12),fishes(jf,14),fishes(jf,15),fishes(jf,16),sedvel(jf),delaydate(jf)
    end do
...
! Compute passive particle transport including sedimentation
    ...
    Data DXMIN,DZMIN1,DZMAX1 /0.0005,0.0001,0.5/   ! SW 2/01/01   in units of m2/s
