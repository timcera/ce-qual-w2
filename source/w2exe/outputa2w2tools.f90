  ...
  IF (TIME_SERIES) THEN  
    ...
    IF(K >= KTWB(JW))THEN      ! SW 4/4/2018 ADDED TO ELIMINATE THE LAST VALUE OF A VARIABLE BEING USED FOR CASE WHEN ESTR IS NEGATIVE
     ...
     ELSE      ! SW 4/4/2018
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),-99.,-99.,QC(I),SRON(JW)*1.06,-99.,DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),ICETH(I),TVOLAVG,rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i),(-99.,JAC=1,NAC),                      &  ! CB 7/26/07
            (-99.,JE=1,NEP),(-99.,JM=1,NMC),SEDCH,SEDPCH,SEDNCH,SEDCCH, &  
        ...
        ELSE  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),-99.,-99.,QC(I),SRON(JW)*1.06,-99.,DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),TVOLAVG,rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i),(-99.,JAC=1,NAC),                      &  ! CB 7/26/07
            (-99.,JE=1,NEP),(-99.,JM=1,NMC),SEDCH,SEDPCH,SEDNCH,SEDCCH, &  
            (-99.,JAD=1,NACD(JW)),(-99.,JF=1,NAF(JW)),(-99.,JA=1,NAL),(-99.,JA=1,NAL),(-99.,JA=1,NAL)    ! SW 10/20/15 
          ELSE  
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),-99.,-99.,QC(I),SRON(JW)*1.06,-99.,DEPTHB(KB(I),I),      &     ! SW 8/13/06  
    ...
    IF (PROFILE(JW)) THEN  
      IF (JDAY >= NXTMPR(JW) .OR. JDAY >= PRFD(PRFDP(JW)+1,JW)) THEN  
        ...
        elseif(jw == 1)then   ! write out individual files on these days
             ...
             IF(CONSTITUENTS)THEN
             WRITE(NUNIT,'(1000(A,","))')'Seg#','ElevWaterSurf(m)','Q(m3/s)','Temp(oC)','Depth(m)','Width(m)',(CNAME2(CN(JC)),JC=1,NAC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW))
             ELSE
             WRITE(NUNIT,'(1000(A,","))')'Seg#','ElevWaterSurf(m)','Q(m3/s)','Temp(oC)','Depth(m)','Width(m)'   
             ENDIF
                 ...
                 DO JJ=1,NWB
                     ...
                     DO JB=BS(JJ),BE(JJ)
                         DO I=CUS(JB),DS(JB)
                             IF(CONSTITUENTS)THEN
                             WRITE(NUNIT,'(I5,",",100(F12.3,","))')I,ELWS(I),QC(I),T2(K,I),DEPTHB(KB(I),I),B(KTI(I),I),(C2(K,I,CN(JAC))*CMULT(CN(JAC)),JAC=1,NAC),(CD(K,I,CDN(JD,JW))*CDMULT(CDN(JD,JW)),JD=1,NACD(JW))
                             ELSE
                            WRITE(NUNIT,'(I5,",",100(F12.3,","))')I,ELWS(I),QC(I),T2(K,I),DEPTHB(KB(I),I),B(KTI(I),I)
                            ENDIF
                         ENDDO
                     ENDDO
                 ENDDO
